...
# Load cylinder mesh file ------------------------------------------------------#
cylinder_file = ((os.getcwd() + os.path.sep + 'cylinder.dat'),) # .dat nemoh, .gdf wamit
cylinder_cg = ((0,0,0),) # center of gravity
cylinder_name = ('cylinder_cpt',) # body name

cylinder_w = np.linspace(0.04, 21.0, 525) # 525 for full, 3 for tests
cylinder_headings = np.linspace(0,0,1)
cylinder_depth = -3.0

cylinder_nc = True
cylinder_ncFile = os.getcwd() + os.path.sep + 'cylinder_full.nc'
# ----------------------------------------------------------------------------#
...
print('\n\nFunction completed. Cylinder data is saved.\n')
