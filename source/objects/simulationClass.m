...
classdef simulationClass<handle
    ...
    properties (SetAccess = 'public', GetAccess = 'public')%input file
        ...
        paraview         = struct(...                                   % (`structure`) Defines the BEM data implemtation. 
            'option' {mustBeMember(reloadH5Data,0:1)}, 0,...            % 
            'startTime' {mustBeNonnegative},	         0, ...           % 
            'endTime' {mustBeNonnegative},             100, ...         % 
            'dt' {mustBePositive},                     0.1, ...         %             
        ...
        pressure {mustBeMember(pressureDis,0:1)}      = 0               % (`integer`) Flag to save pressure distribution, Options: 0 (off), 1 (on). Default = ``0``
        ...
        reloadH5Data {mustBeMember(pressureDis,0:1)}  = 0               % (`integer`) Flag to re-load hydro data from h5 file between runs, Options: 0 (off), 1 (on). Default = ``0``
        rho {mustBePositive}                          = 1000            % (`float`) Density of water. Default = ``1000`` kg/m^3
        saveStructure {mustBeMember(pressureDis,0:1)} = 0               % (`integer`) Flag to save results as a MATLAB structure, Options: 0 (off), 1 (on). Default = ``1``
        saveText {mustBeMember(pressureDis,0:1)}      = 0               % (`integer`) Flag to save results as ASCII files, Options: 0 (off), 1 (on). Default = ``0``
        saveWorkspace {mustBeMember(pressureDis,0:1)} = 1               % (`integer`) FLag to save .mat file for each run, Options: 0 (off), 1 (on). Default = ``1``
        ...
        stateSpace {mustBeMember(pressureDis,0:1)}    = 0               % (`integer`) Flag for convolution integral or state-space calculation, Options: 0 (convolution integral), 1 (state-space). Default = ``0``
    ...
    methods
        ...
        function setup(obj)
            ...
            if isempty(obj.dtOut) || obj.dtOut < obj.dt
                obj.dtOut = obj.dt;
                warning('Simu.dtOut, the output timestep, should be greater than or equal to simu.dt, the simulation timestep. The output timestep has been overwritten with the value of the simulation timestep to avoid errors.')
            end            
