...
classdef responseClass<handle
    ...
    % ``waveClass``, ``bodyClass``, ``constraintClass``, ``controllerClass``, ``ptoClass``, 
    ...
    %
    %.. autoattribute:: objects.responseClass.controllers
    %    
    %   * ``name`` (`string`) = 'controllerName'
    %   * ``time`` (`array`) = [# of time-steps x 1]
    %   * ``force`` (`array`) = [# of time-steps x 6] The force applied by the controller
    %   * ``power`` (`array`) = [# of time-steps x 6] The power generated by the controller
    %
    ...
    properties (SetAccess = 'public', GetAccess = 'public')
        ...
        controllers         = struct()     % This property contains a structure for each instance of the ``controllerClass`` (i.e. for each Controller block)
    ...
    methods (Access = 'public')
        function obj = responseClass(bodiesOutput,ptosOutput,controllersOutput,constraintsOutput,ptosimOutput,cablesOutput,mooringOutput,waveOutput) 
            ...
            if isstruct(controllersOutput)
                signals = {'force','power'};
                for ii = 1:length(controllersOutput)
                    obj.controllers(ii).name = controllersOutput(ii).name;
                    obj.controllers(ii).time = controllersOutput(ii).time;
                    for jj = 1:length(signals)
                        obj.controllers(ii).(signals{jj}) =  controllersOutput(ii).signals.values(:,(jj-1)*6+1:(jj-1)*6+6);
                    end
                end
            end
            % Constraints
