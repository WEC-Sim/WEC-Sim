function hydro = Read_CAPYTAINE_b2b(hydro,filename)
...
% standard capytaine dof naming based on number of bodies
std_dofs = ["surge", "sway", "heave", "roll", "pitch", "yaw"];
tmp = [];
if length(hydro.body)>1
    for i=1:length(hydro.body)
        tmp = [tmp strcat(hydro.body{i},'__',std_dofs)];
    end
    std_dofs = tmp';
end
clear tmp
...
tmp = true;
for i=1:length(rdofs)
    tmp = tmp && strcmp(rdofs(i), std_dofs(i));
end
reset_rdofs = ~tmp;
clear tmp
...
for i=1:length(idofs)
    tmp = tmp && strcmp(idofs(i), std_dofs(i));
end
reset_idofs = ~tmp;
clear tmp
...
function new_var = reset_dofs(variable, old_dofs, dim, body_names)
