...
In Option 1, the sub-surface current is depth indepent and is equal to the current at the water surface:

.. math::
   U_{ss,current}(z) = U_{ss,current}(0)\left[ \cos \left(\theta_{c} \right) + \sin \left(\theta_{c} \right)\right]
...
In Option 2, the sub-surface currert profile varies with depth following a 1/7th power law:
    
.. math::
   U_{ss,current}(z) = U_{ss,current}(0)\left[ 1 + \frac{z}{d_{current}} \right]^{1/7} \left[ \cos \left(\theta_{c}\right) + \sin \left(\theta_{c} \right)\right]
...
In Option 3, the sub-surface currert profile varies linearly with depth:
    
.. math::
   U_{ss,current}(z) = U_{ss,current}(0)\left[ 1 + \frac{z}{d_{current}} \right] \left[ \cos \left(\theta_{c} \right) + \sin \left(\theta_{c} \right)\right]

.. Note:: 
    WEC-Sim does not adjust the linear wave hydrodynamics based on the presence 
    of ocean currents and assumes a linear supperposition between current and
