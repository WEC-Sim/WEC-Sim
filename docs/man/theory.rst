...
WEC-Sim imports non-dimensional hydrodynamic coefficients from an ``*.h5``  data structure generated by :ref:`bemio` for the WAMIT, AQWA or Nemoh BEM solvers. 
...
where :math:`t_{k}=k\Delta t` for :math:`k=0,~1,~2,~\ldots` with :math:`\Delta t` being the sampling period.  The feed-through matrix :math:`\mathbf{D_d}` is assumed to be zero in order to maintain causality of the system, as a non-zero :math:`\mathbf{D_d}` results in an infinite value at :math:`t=0`.
...
The normalizing factor, :math:`C\left(\gamma\right)`, is defined as: 
