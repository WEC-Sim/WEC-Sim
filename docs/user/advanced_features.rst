...
Irregular Waves with AQWA Sign Convention
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Users can override the WEC-Sim sign convention to AQWA sign convention.
To enable this correction, add the following to your 
``wecSimInputFile``:: 

    waves.aqwaConvention = 1
