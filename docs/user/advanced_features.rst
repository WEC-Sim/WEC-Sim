...
``output.plotResponse``, ``output.plotWaveBodyVisualization``, ``waves.plotEta``, and
...
Plot Wave-Body Visualization
^^^^^^^^^^^^^^^^^^^^^^^^^^^^

The ``responseClass.plotWaveBodyVisualization()`` method can be used to create a complete 
...
``output.plotWaveBodyVisualization(simu,body,waves,'timesPerFrame',5,'axisLimits',[-50 50 -50 50 -12 20])``
...
.. figure:: /_static/images/OSWEC_plotWaves.PNG
   ...
   Demonstration of output.plotWaveBodyVisualization() method for the OSWEC example.   
