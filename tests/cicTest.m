classdef cicTest < matlab.unittest.TestCase
    ...
    methods (Access = 'public')
        function obj = cicTest()
            ...
            irkbInput = zeros(length(cicTime), nDOF, lDOF);
            for j = 1:2 % 2 variable hydro states
                for i = 1:6
                    irkbInput(:, i, i, j) = a*j; % scale the base IRF_k across variable hydro states
                end
            end
    ...
    methods(TestClassSetup)
        function calcForceRad(testCase)
            ...
            for it = 1:size(testCase.velocity,1)
                testCase.Frad0(it,:) = ConvolutionIntegral_interp(testCase.velocity(it,:), testCase.irkbInput(:,:,:,1), testCase.cicTime);
            end
    ...
    methods(Test)
        function compareComputationalTime(testCase)
            ...
            fprintf('Original function: %.4f \n', testCase.t0);
            fprintf('Surface function without switching: %.4f \n', testCase.ts1);
            fprintf('Surface function with switching: %.4f \n', testCase.ts2);
        end

        function compareMethods(testCase)
            testCase.assertEqual(testCase.Frad0, testCase.FradS1);
        end

        function compareStateSwitching(testCase)
